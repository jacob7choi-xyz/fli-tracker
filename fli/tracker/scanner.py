...
def _search_duration(
    ...
    segments, trip_type = build_date_search_segments(
        ...
        trip_duration=duration if route.is_round_trip else None,
    ...
    try:
        ...
        logger.exception(
            "Search failed for route %s -> %s (duration=%d)",
            route.origin,
            route.destination,
            duration,
        )
...
def scan_route(route: Route) -> list[PriceSnapshot]:
    ...
    Results are deduplicated by (departure_date, return_date, price) to
    avoid storing identical itineraries found across different duration searches.
    ...
    logger.info(
        "Found %d prices for route %s -> %s (%d duration(s))",
        len(all_snapshots),
        route.origin,
        route.destination,
        len(route.durations),
